<?php

require_once("config/config.php");

//Assign default values
$signatureClient = null;
$htmlFooter = '';

$clientEmail = 'client@example.com';
$emailDev = 'dev@example.com';
$signatureDev = 'data:image/png;base64, ';
$signatureDev = '<img id="dev_signature" src="' . $signatureDev . '" >';
$signatoryName = 'Trevor Belomont';

/** Functionality to self delete as required for test purposes */
$selfDelete = 0;

$path = 'libs/dompdf/toPrint/signedContracts/toPrint/';
$fileName = sha1($_SESSION['timeStart']);
$htmlName = $fileName . '.html';
$pdfName = $fileName . '.pdf';


/** Check if client has signed */
//Assign default value to signature

//Set signature variable, from $_POST, if client signed
if(isset($_POST['client_signature']))
{
    $signatureClient = $_POST['client_signature'];

    //Make sure the signature is a base64 encoded png and there is a minimum signature size
    if(substr($signatureClient, 0, 22) === 'data:image/png;base64,' && strlen($signatureClient) > 200)
    {
        $signatureClient = '<img id="hk" src="' . htmlspecialchars($signatureClient) . '" >';
    }
}

function getHeader($signed = false)
{
    global $html_content, $htmlFooter;

    $page_title = PAGE_TITLE;

    $icon = "fas fa-times-circle";
    $color = "#ed1c24";

    if($signed)
    {
        $css_file = '<link rel="stylesheet" type="text/css" href="css/style.css">';
        $page_title = "Signed Contract";
        $icon = "fas fa-check-circle";
        $color = "#2bb673";
    }
    else
    {
        $css_file = '<link rel="stylesheet" type="text/css" href="assets/css/style.css">';
    }

    $logo = 'data:image/png;base64, ';

    $html = '
        <!DOCTYPE html>
            <html lang="en">
            <head>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1"/>
                <meta name="robots" content="noindex">
                ' . $css_file . '
                <title>' .$page_title. '</title>
            </head>
            
            <body>
                <div class="tool_bar noprint">
                    <div class="logo">
                        <img class="logo_img" src="'.$logo.'" alt="logo" />
                    </div>
                    <div class="title_container">
                        <h4 class="title">DocuSigner <span class="beta_tag">BETA</span></h4>
                    </div>
                    <div class="doc_title_container" aria-label="Thank you for signing: Contract of Work for Website Design and Development">
                        <h4 class="doc_title"><span style="color: '.$color.'; font-size: 18px;"><i class="'.$icon.'"></i></span> Contract of Work for Website Design and Development</h4>
                    </div>
                    <div class="docs_icon">
                        <div class="tooltip">
                            <a href="test.php">
                                <i class="fas fa-file-signature" style="font-size: 24px; padding: 5px; margin-top: 3px; color: #5f6368"></i>
                            </a>
                          <span class="tooltiptext tooltip-bottom">My Documents</span>
                        </div> 
                       
                    </div>
                    <div class="avatar">
                        <div class="tooltip">
                            <a class="av_link" onclick="$(\'.account_panel\').toggle();">
                                <img class="av_image" src="https://www.propertycoza.com/images/roundLogo.png" alt="avatar" />
                            </a>
                            <span class="tooltiptext tooltip-bottom">Property.CoZa Account</span>
                        </div>
                    </div>  
                </div>
                <div class="account_panel">
                    <div class="account_avatar_panel">
                        <div class="account_avatar">
                            <img class="acc_avatar" src="https://www.propertycoza.com/images/roundLogo.png" alt="avatar" />
                        </div>
                        <div class="acc_info">
                            <div class="acc_title">Head Office</div>
                            <div class="acc_mail">admin@dotcomnet.co.za</div>
                        </div>
                        <div class="logout_container">
                            <a href="#" class="logout_button">Sign Out</a>
                        </div>
                    </div>
                </div>
                <div class="page-wrapper" id="content">
                  
    ';

    $html .= $html_content;

    $html .= $htmlFooter;

    return $html;
}

function generateJS($signed = false)
{
    global $fileName;

    if($signed)
    {
        return '
            <script>
                function printContract() {
                  window.print();
                }
                function generatePdf() {
                    window.location.href = "../../../../../print.php?document=' . $fileName . '";
                }
            </script>
            <script src="js/jquery-3.6.0.min.js"></script>
            <script src="https://kit.fontawesome.com/bfd9897846.js" crossorigin="anonymous"></script>
            ';
    }
    else
    {
        return '
            <script src="./assets/js/jquery-3.6.0.min.js"></script>
            <script src="https://kit.fontawesome.com/bfd9897846.js" crossorigin="anonymous"></script>
            <!-- https://github.com/brinley/jSignature/blob/master/README.md -->
            <script src="./assets/js/jSignature.min.js"></script>
            <script src="./assets/js/core.js"></script>
            ';
    }
}

function getFooter($signed = false)
{
    global $htmlFooter, $signatureClient, $signatureDev, $signatoryName;

    if ($signed)
    {
        $_SESSION['timeEnd'] = microtime(true);
        $browserName = functions::getBrowser();
        $signedOn = date('j F Y g:i a');
        $clientIP = functions::get_client_ip_env();

        $js = generateJs($signed);

        $htmlFooter = $signatureDev;
        $htmlFooter .= $signatureClient;

        $htmlFooter .= '
              <div id="date-ip">
                <strong>Signed on:</strong> ' . $signedOn .' <br>
                <strong>IP address:</strong> ' . $clientIP .'<br>
                <strong>Browser:</strong> ' . $browserName .'<br>
                <strong>PC Name:</strong> ' . gethostname() .'<br>
                <strong>Operating System:</strong> ' . PHP_OS .'<br>
                <strong>Time Spent:</strong> '. substr((($_SESSION["timeEnd"] - $_SESSION["timeStart"]) / 60) ,0,4) . ' Minutes<br>
              </div>
            
              <div class="noprint" id="print-pdf">
                <button id="print" type="button" class="btn-primary" onclick="printContract()">
                    Print contract
                </button>
                <button id="pdf" type="button" class="btn-primary" onclick="generatePdf()">
                    Download as PDF
                </button>
              </div>
          </div> <!-- #doc_container -->
            
        </div> <!--#content-->
          
            ' . $js . '
        </body>
        </html>';
    }
    else
    {
        $js = generateJs();


        $htmlFooter = $signatureDev;
        $htmlFooter .= '
                  <form method="post" class="noprint" id="signature_form">
                    <div id="signature">
                      <!-- Client Signature Canvas -->
                    </div>
                    <div id="signatory">' . $signatoryName . '</div>
                
                    <div class="buttons">
                      <button class="btn-primary" id="reset" type="button">Reset</button>
                      <button class="btn-primary" id="submit" type="submit">Done <i class="fas fa-arrow-right"></i></button>
                      <p>';

                        $htmlFooter .= date('d-m-Y h:m');

                        $htmlFooter .= '
                    </p>
                    </div>
                
                    <input type="hidden" id="client_signature" name="client_signature" />
                  </form>
              </div> <!-- #doc_container -->
            
            </div> <!-- #content -->
               
                ' . $js . '
            </body>
            </html>';
    }
}

function setHtmlContent()
{
    global $html_content;

    $html_content = file_get_contents(ROOT_PATH . '/_templates/Contract of work for website design and development.html');
}

function saveHtml()
{
    global $path, $htmlName, $html_content, $signatureClient, $signatureDev;

    $html = getHeader(true);
    $htmlFileLocation = $path . $htmlName;
    file_put_contents($htmlFileLocation, $html);
//    file_put_contents($htmlFileLocation, $html_content, FILE_APPEND | LOCK_EX);
//    file_put_contents($htmlFileLocation, $signatureDev, FILE_APPEND | LOCK_EX);
//    file_put_contents($htmlFileLocation, $signatureClient, FILE_APPEND | LOCK_EX);
//    file_put_contents($htmlFileLocation, $html, FILE_APPEND | LOCK_EX);

//    ob_start();
    /*    eval (' ?>' . $html_footer_signed . '<?php ');*/
//    $FOOTER_SIGNED_COMPILED = ob_get_contents();
//    ob_end_clean();

//    file_put_contents($htmlFileLocation, $FOOTER_SIGNED_COMPILED, FILE_APPEND | LOCK_EX);

    return $htmlFileLocation;
}

setHtmlContent();

if($signatureClient)
{
    getFooter(true);
    getHeader(true);
}

if($signatureClient == null)
{
    if($selfDelete && file_exists($htmlName))
    {
        header('Location: ' . $htmlName . '#hk');
        die();
    }

    /** Waiting for Client to sign: include signature elements and javascript **/
    getFooter();
    echo getHeader();
}
else
{
    /** Contract was just signed: put $signatureClient and the other parts in the .html file **/
    $htmlFileLocation = saveHTML();

    functions::emailMembers();

    if($selfDelete)
    {
        unlink(__FILE__);
    }

    session_destroy();
    header('location: ' . $htmlFileLocation . '#hk');

    die();
}

